...
public class Node : Item {
    ...
    public IEnumerable<Item> Children { set; get; }

    public override string ToString() {
        if (Children != null) {
            return string.Format("<{0}>", Name) +
                Children.Aggregate("", (s, c) => s + c) + string.Format("</{0}", Name);
        }
        return string.Format("<{0}/>", Name);
    }
}
...
class XmlParser {
    ...
    static Parser<string> EndTag(string name) {
        return Tag(from slash in Parse.Char('/')
                   ...
                   where id == name
                   select id).Named("closeing tag for " + name);
    }
...
var input = File.ReadAllText("resource/Hello.xml");
